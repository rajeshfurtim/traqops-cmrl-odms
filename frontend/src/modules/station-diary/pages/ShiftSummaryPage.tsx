import { BookOpen, Check, Copy, Download, FileSpreadsheet, FileText, Table2 } from 'lucide-react'
      ...
      <div className="flex flex-col gap-4">
            ...
            {mode === 'table' ? (
              <>
                ...
                <Button
                  ...
                  onClick={() => downloadCsv(filtered, `${stationCode}_ShiftSummary_${from}_to_${to}.csv`)}
                >
                  Excel
                </Button>
              </>
        ...
        {filtered.length === 0 ? (
                  ...
                  {visible.map((d) => (
                    <tr key={d.id} className="hover:bg-subtle/60">
                      ...
                      <td className={TD}>
                        ...
                        {d.handover.handedAt && (
                          <span className="block text-caption text-ink-muted">{formatStamp(d.handover.handedAt)}</span>
                        )}
                      </td>
                      <td className={TD}>
                      ...
                      <td className={`${TD} text-right whitespace-nowrap`}>
                        ...
                        {d.status !== 'no-attendance' && (
                          <Button size="sm" variant="ghost" icon={FileText} onClick={() => setExportFor({ diary: d })}>
                            PDF
                          </Button>
                        )}
                      </td>
          ...
          <section aria-label="Booklet" className="flex flex-col gap-4">
            ...
            <div className="flex flex-col items-center gap-5 rounded-xl bg-muted px-3 py-5 sm:px-6">
              {visible.map((d, i) => (
                <DiaryFormSheet
                  key={d.id}
                  diary={d}
            ...
            <Pagination page={current} pageCount={pageCount} onChange={setPage} withEnds className="self-center" />
          </section>
        )}
      </div>
