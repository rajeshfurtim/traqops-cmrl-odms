...
- A module declares a `ReportDefinition` (columns, file name, default header) and renders
  `<ExportActions report rows rowKey period filters />`. Copy and CSV are data only; Excel, PDF and Print open a
  preview. Export takes every row that matches the filters, not only the visible page.
- **Header templates:** users can change only the **Layout** (Standard / Compact), **Register title** (default
  `{register.name}`), **Subtitle** and **Department / section** (stored as data, never code). Everything else is
  common and fixed on every document: CMRL logo, organisation name, run details (station, period, filters, records,
  generated by / on), watermark and ODMS footer. Per report, code may fix
  `header.formRef` / `revision` / `issueDate` / `orientation` in the definition. Placeholders such as `{station.name}`,
  `{register.name}`, `{period.from}` (full list in `header/placeholders.ts`) are filled in as plain text; unknown ones
  block saving.
- Which header is used: station override → saved register default → the register's built-in default
  (`header/resolveHeader.ts`). A station override replaces the default as a whole and warns when the default changes later.
- Every save adds a version (`data/templateStore.ts`, append-only, with who, when and a note). Each export prints its
  template reference in the footer (e.g. `Header OCC/CEN01 v3`), stores it in the PDF/Excel properties and is logged
  with `recordExport`.
- The preview dialog allows one-time header changes; **Save as template** is separate. Editing is permission-checked
  with `can()` in `utils/permissions.ts` (Station Controller: station override; Supervisor/Admin: register default).
  The user menu has a **Role (demo)** switch until the backend sends permissions.
- `layout.ts` turns data + header into one `ReportLayout`; the HTML sheet (preview and print), the PDF
  (`@react-pdf/renderer`) and the Excel header (ExcelJS, MIT) all draw from it. PDF and Excel code load only on export.
